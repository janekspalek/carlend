<?php
    ...
    $zprava = "";
    if(isset($_POST["zmena-jmena"])) {
        $pripojeni = mysqli_init();
        if(!mysqli_real_connect($pripojeni, $server, $uzivatel, $heslo, $db)) {
            die("Chyba při připojování k databázi");
        }
        else {
            $sql = "UPDATE uzivatele SET jmeno='" . $_POST["jmeno"] . "', prijmeni='" . $_POST["prijmeni"] . "' WHERE id_u = " . $_SESSION["id"];
            mysqli_query($pripojeni, $sql);
            header("Location: moje-udaje.php?zmena=1");
        }
    }
                ...
                if(isset($_SESSION["id"])) {
                    ...
                    if(mysqli_num_rows($uzivatel) > 0) {
                        ...
                        <div class="container mx-auto w-full mb-8 overflow-hidden rounded-xl shadow-lg bg-white p-20 px-32">';
                            ...
                            echo '
                            <form action="moje-udaje.php" method="post" class="grid grid-cols-2 w-[1000px]">

                                <p class="font-bold mb-2">Email: </p>
                                <input type="text" name="email" class="p-2 mb-4 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg w-full" value="' . $uzivatel["email"] . '" required readonly>
                                ...
                                <input type="hidden" name="zmena-jmena" value="1">
                                
                                <p></p>
                                <div>
                                    <button type="submit" class="py-2 w-52 mr-8 mb-10 bg-red-500 hover:bg-red-400 rounded-full transition-all duration-200 text-white">Uložit</button>
                                </div>
                            </form>
                            ...
                            <form action="moje-udaje.php" method="post" class="grid grid-cols-2 w-[1000px]">
                                ...
                                <p></p>
                                <div>
                                    <button type="submit" class="py-2 w-52 mr-8 bg-red-500 hover:bg-red-400 rounded-full transition-all duration-200 text-white">Změnit heslo</button>
                                </div>
                            </form>
                        </div>';
                    }
